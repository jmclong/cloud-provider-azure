...
func TestCreateManagedDisk(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:                "disk Id and no error shall be returned if PremiumV2_LRS accepts IOPS and MBPS parameters",
			diskID:              disk1ID,
			diskName:            disk1Name,
			storageAccountType:  compute.DiskStorageAccountTypesPremiumV2LRS,
			diskIOPSReadWrite:   "100",
			diskMBPSReadWrite:   "100",
			diskEncryptionSetID: "",
			expectedDiskID:      "",
